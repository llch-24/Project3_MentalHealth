...
combined_df %>%
  ...
  geom_text(check_overlap = TRUE, color = "red", size = 4.5, hjust = -0.1, vjust = 0.5) +
  labs(
...
Closer Look into 5 models and notice how similar they are
```{r}
### extract all of the resample fold performance metrics per model
concrete_model_compare_lf <- concrete_model_compare$values %>% tibble::as_tibble() %>% 
  pivot_longer(!c("Resample")) %>% 
...
### and standard error, which model is better?
concrete_model_compare_lf %>% 
  filter(model_name == "model_6_linear" | model_name == "model_7_linear" | model_name == "Elastic_Net_model_06" | model_name == "Elastic_Net_model_07" | model_name == "RF_model_7") %>% 
  ggplot(mapping = aes(x = model_name, y = value)) +
  geom_point() +
  stat_summary(fun.data = "mean_se",
               color = "red",
